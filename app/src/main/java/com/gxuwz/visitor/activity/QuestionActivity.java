...
public class QuestionActivity extends BaseActivity {
    ...
    private void initQuestion() {
        MyApplication.appExecutors.diskIO().execute(() -> {
            if (intent != null) {
                String subjects = intent.getStringExtra("subject");
                if (subjects != null) {
                    // TODO: 2024/6/28  查询数据库
                    questions = db.questionDao().getAllQuestionsBySubject(Integer.parseInt(subjects));
                    Log.d("TAG", "initQuestion: " + questions.size());
                    new Handler(Looper.getMainLooper()).post(() -> displayQuestion(currentQuestionIndex));
                } else {
                    Log.e("QuestionActivity", "Subject is null");
                    // 处理 subjects 为 null 的情况
                }
            } else {
                Log.e("QuestionActivity", "Intent is null");
                // 处理 intent 为 null 的情况
            }
        });
    }
                    ...
                    if (selectedIndex == Integer.parseInt(questions.get(currentQuestionIndex).getAnswer()) - 1) {
                        ...
                        MyApplication.appExecutors.diskIO().execute(() -> {
                            ...
                            if (db.questionDao().getQuestionById(questions.get(currentQuestionIndex).getId()) == null) {
                                ErrorQuestion errorQuestion = new ErrorQuestion(questions.get(currentQuestionIndex), 1);
